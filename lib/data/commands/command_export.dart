...
/// 내보낸 결과가 만든 파일 수(안내 문구용). 명령 파일 하나와 동봉 이미지들이다.
...
/// [exported]를 **명령 배열 파일 하나**로 쓰고, 참조하는 이미지 캐시 파일을 그
/// **옆에** 같은 이름으로 복사한다.
///
/// 받는 쪽은 이 파일들을 함께 옮겨 콘솔의 `import`로 먹으면 된다 — 적용이 이미지의
/// 상대 경로를 **명령 파일이 놓인 폴더** 기준으로 찾으므로 나란히 있으면 그대로 맞는다.
///
/// 이미지를 먼저 복사하고 명령 파일을 **마지막에** 쓴다. 파일이 다 놓이기 전에 누가
/// 읽어 가면 아직 없는 이미지를 찾아 실패하는데, 순서를 이렇게 두면 명령 파일이
/// 나타난 시점에는 이미지가 이미 자리에 있다.
