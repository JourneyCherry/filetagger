/// 명령 적용이 DB 밖(파일시스템)에 물어봐야 하는 것들. 구현은 data 계층에 격리한다.
///
...
abstract interface class CommandEnvironment {
  ...
  ///
  /// 밖에서 들어오는 명령은 파일을 먼저 쓴 뒤 태그를 걸기로 약속되어 있으므로, 이
  /// 답이 false면 곧 나타나기를 기다리지 않는다.
  Future<bool> targetExists(String relPath);

  /// 이미지 파일 [externalPath]를 썸네일 캐시에 등록하고 저장할 캐시 키를
  /// 돌려준다. 이미지가 아니거나 읽지 못하면 null.
  ///
  /// **절대 경로면 그 파일을, 상대 경로면 부르는 쪽이 정한 기준 폴더**에서 찾는다.
  /// 상대 경로를 받는 것은 내보내기가 명령 파일 옆에 이미지를 함께 두기 때문이다 —
  /// 그래야 파일 몇 개를 통째로 옮기는 것만으로 이관이 성립한다. 기준이 어디인지는
  /// 명령이 어디서 왔는지에 달렸으므로 구현이 받아 둔다.
