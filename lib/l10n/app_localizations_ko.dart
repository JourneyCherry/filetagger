...
class AppLocalizationsKo extends AppLocalizations {
  ...
  String get helpTopicManageModeBody =>
      '폴더는 \"폴더만 관리\"(하나의 항목으로만 다루고 내부를 목록에 내지 않음), \"내부 관리\"(직속 내용만), \"재귀적으로 관리\"(하위까지 이어서) 중 하나로 다룹니다. 따로 정하지 않은 폴더는 상위의 방식을 물려받습니다. 큰 폴더의 내부를 인덱싱하지 않으려면 관리 방식을 바꾸세요. 내부를 관리하지 않는 폴더도 바로 안에 이미지가 있으면 그것을 폴더 썸네일로 씁니다.';
  ...
  String get helpSystemTagAspectRatio =>
      '이미지의 가로÷세로 비율입니다. 가로와 세로를 모두 읽을 수 있는 이미지에만 붙습니다. 가로로 길수록 값이 크므로 오름차순은 세로형 → 가로형, 내림차순은 가로형 → 세로형 순으로 늘어섭니다. 픽셀 수가 조금 달라도 비율이 같으면 같은 값으로 접히므로, 이 태그로 묶으면 가로형·세로형·정사각형이 한 덩어리씩 모입니다. 화면에는 16:9처럼 가로:세로로 보이지만 견주는 값은 나눗셈 결과인 소수이므로, 필터 칸에는 소수로 적습니다.';
  ...
  String get tipSubtitleTagBody =>
      '이름 아래 줄에는 기본으로 경로가 보입니다. 태그로 묶어 보는 동안에는 경로보다 작가·연도처럼 다른 축이 더 궁금할 수 있는데, 그때 부제 태그를 지정하면 그 값이 대신 보입니다. 이름 태그와 같은 방식이라 여러 개를 순서대로 둘 수 있고, 그 태그가 없는 항목은 경로를 그대로 보입니다. 직접 만든 태그뿐 아니라 수정시각·확장자·이미지 크기 같은 시스템 태그도 고를 수 있습니다.';
  ...
  @override
  String get tipHiddenTagPreviewTitle => '감춘 태그값 확인하기';

  @override
  String get tipHiddenTagPreviewBody =>
      '태그 관리에서 눈 아이콘을 끄면 목록 행의 칩만 사라집니다. 값은 그대로 남아 필터·정렬·그룹에 쓰이고, 프리뷰 창에는 감춘 태그와 표시로 켜지 않은 시스템 태그까지 모두 보입니다. 한 항목에 무엇이 붙어 있는지 전부 보려면 프리뷰를 여세요.';
