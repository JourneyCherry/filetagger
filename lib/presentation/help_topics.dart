...
const List<HelpTopic> helpTopics = [
  ...
  HelpTopic(
    ...
    body:
        ...
        '나중에 태그 이름을 바꿔도 조건이 끊기지 않습니다. 정렬 캡슐을 누르면 '
        '오름차순·내림차순·무작위를 차례로 오갑니다 — 무작위는 그 태그값의 순서만 '
        '흩고 값이 같은 항목끼리는 건드리지 않아, 뒤 단계의 정렬이 그대로 살아 '
        '있습니다. 무작위를 다시 고를 때마다 새로 섞입니다.',
  ...
  HelpTopic(
    title: '글자를 쳐서 항목 찾기',
    body:
        '목록에 포커스가 있을 때 글자를 치면 그 글자로 시작하는 항목으로 커서가 건너뜁니다'
        '(이름 칸에 보이는 이름 기준이라, 이름 태그로 갈아 낀 이름이 있으면 그 이름으로 '
        '찾습니다). 이어서 치면 검색어가 좁혀지고, 같은 글자를 거듭 치면 그 글자로 '
        '시작하는 항목들을 차례로 돕니다. 잠시 멈추면 검색어가 지워져 다음 글자가 새 '
        '검색이 됩니다. 목록을 줄이는 것이 아니라 커서만 옮기는 것이라, 필터를 걸어 둔 '
        '채로도 그대로 쓸 수 있습니다. 접어 둔 그룹이나 폴더 안에 있어 지금 보이지 않는 '
        '항목도 찾아냅니다 — 필터에 걸러지지 않았다면 대상이며, 찾으면 그 항목에 이르는 '
        '그룹·폴더만 펼쳐 드러냅니다. 세 보기 모드에서 모두 되고, 아이콘 보기는 지금 '
        '파고든 계층 안에서만 찾습니다.',
...
String systemTagDescription(SystemTag tag) => switch (tag) {
  ...
  SystemTag.imageWidth =>
    '이미지의 가로 픽셀 수입니다. 크기를 읽을 수 있는 '
        '이미지에만 붙으므로, 이 태그를 "있음"으로 걸러 이미지만 모을 수 있습니다. '
        '숫자라서 큰 것부터 정렬하거나 "얼마 이상"으로 거를 수 있습니다.',
  SystemTag.imageHeight =>
    '이미지의 세로 픽셀 수입니다. 가로와 따로 붙어(둘은 늘 '
        '함께 붙거나 함께 없습니다), 가로·세로를 각각 정렬 기준이나 조건으로 쓸 수 '
        '있습니다.',
  SystemTag.fileName =>
    '항목의 이름입니다. 시스템 태그 중 유일하게 값을 고칠 '
        '수 있고, 고치면 디스크의 실제 이름이 바뀝니다(키워드는 디스크에 실체가 없어 '
        '키워드의 이름만 바뀝니다).',
  SystemTag.childFileCount =>
    '폴더가 바로 아래에 담고 있는 파일의 수입니다. 하위 폴더와 '
        '그 안의 파일은 세지 않습니다. 폴더에만 붙으므로 폴더 표식을 겸해, 필터에서 '
        '"있음"으로 폴더만, 제외로 파일만 남길 수 있습니다(빈 폴더도 0이 붙습니다). '
        '내부를 감춘 폴더에도 붙어, 열어 보지 않고 안에 무엇이 얼마나 있는지 가늠하거나 '
        '수량으로 정렬할 수 있습니다.',
  SystemTag.keyword =>
