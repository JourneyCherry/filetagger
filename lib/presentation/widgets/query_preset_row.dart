...
/// 도구모음 맨 위 줄에 저장된 조건 프리셋을 캡슐로 늘어놓는다.
///
/// 탭하면 그 프리셋의 조건을 통째로 걸고(지금 걸린 조건은 모두 지워진다), 손잡이
/// 드래그로 순서를 바꾸며, x로 지운다. 이름 변경·덮어쓰기는 우클릭(모바일은 길게
/// 누르기) 메뉴에 둔다. 지우기는 어느 길로 오든 확인을 한 번 거친다 — 조건 칩과 달리
/// 사용자가 만들어 둔 자산이라 한 번의 오조작으로 사라지면 되돌릴 수 없다.
class QueryPresetRow extends ConsumerWidget {
  ...
  /// 프리셋의 조건을 걸고, 태그가 사라져 걸 수 없던 조각이 있으면 알린다.
  void _apply(BuildContext context, WidgetRef ref, QueryPreset preset) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('없는 태그를 가리키는 조건 $dropped개는 빼고 불러왔습니다.')),
    );
  }
...
enum _PresetMenuAction {
  ...
  overwrite('현재 조건으로 덮어쓰기', Icons.save_as_outlined),
...
/// 프리셋이 담은 조건을 한눈에 보는 여러 줄 요약(툴팁용). 조건 줄의 텍스트 표현을
...
class _PresetSaveDialogState extends ConsumerState<_PresetSaveDialog> {
  ...
  Widget build(BuildContext context) {
    ...
    return escDismissible(
      context,
      AlertDialog(
        title: const Text('현재 조건을 프리셋으로 저장'),
        content: dialogContentBox(
          context,
          width: 420,
          child: Column(
  ...
  Widget _preview(String label, List<Widget> chips) {
