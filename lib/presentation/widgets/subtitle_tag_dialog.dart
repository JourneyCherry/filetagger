...
/// 목록의 **부제 줄에 보일 값의 출처 우선순위**를 편집하는 다이얼로그.
///
/// 이름 태그와 같은 얼개다 — 한 노드는 이 순서로 훑어 **처음으로 글자를 낸 태그**의
/// 값을 부제 자리에 보이고, 어느 태그도 못 내면 **경로**로 폴백한다. 그래서 기본은 늘
/// 맨 아래에 있는 셈이라 목록에 두지 않는다. 편집은 즉시 저장된다.
///
...
class _SubtitleTagDialog extends ConsumerStatefulWidget {
  const _SubtitleTagDialog();

  @override
  ConsumerState<_SubtitleTagDialog> createState() => _SubtitleTagDialogState();
}

class _SubtitleTagDialogState extends ConsumerState<_SubtitleTagDialog> {
  /// 우선순위 목록(앞이 높음). 태그 id만 담는다 — 경로는 늘 맨 아래(암묵적).
  late final List<int> _order = [...ref.read(subtitleSourcesProvider)];

  void _save() =>
      ref.read(viewSettingsProvider.notifier).updateSubtitleSources(_order);
