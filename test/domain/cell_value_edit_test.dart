...
void main() {
  group('formatCellEditText', () {
    test('단일 텍스트는 인용 없이 그 값만', () {
      expect(formatCellEditText(TagValueType.text, false, ['a b c']), 'a b c');
    });
    ...
    test('날짜는 사람이 읽는 형식으로 자른다', () {
      expect(
        formatCellEditText(TagValueType.date, false, [
  ...
  group('parseCellEditText', () {
    test('단일 텍스트는 콤마도 값의 일부로 통째로 받는다', () {
      expect(parseCellEditText(TagValueType.text, false, 'a, b'), ['a, b']);
    });

    test('다중 텍스트는 인용된 콤마 구분을 나눠 인용을 푼다', () {
      expect(parseCellEditText(TagValueType.text, true, '"foo bar", "baz"'), [
        'foo bar',
        'baz',
      ]);
    });
    ...
    test('인용 안 한 조각도 콤마로 나눠 받는다', () {
      expect(parseCellEditText(TagValueType.text, true, 'foo, bar'), [
        'foo',
        'bar',
      ]);
    });
    ...
    test('겹친 쌍따옴표는 한 개로 되돌린다', () {
      expect(parseCellEditText(TagValueType.text, true, '"say ""hi"""'), [
        'say "hi"',
      ]);
    });
    ...
    test('빈 조각은 버린다(빈 입력은 태그 제거)', () {
      ...
      expect(parseCellEditText(TagValueType.text, true, '"a", , "b"'), [
        'a',
        'b',
      ]);
    });
    ...
    test('숫자가 아닌 조각은 버린다', () {
      expect(parseCellEditText(TagValueType.number, true, '1, x, 3'), [
        '1',
        '3',
      ]);
    });
    ...
    test('날짜는 저장 형식(시각 제거)으로 바꾸고 못 읽으면 버린다', () {
      expect(parseCellEditText(TagValueType.date, true, '2026-07-13, nope'), [
        '2026-07-13T00:00:00.000',
      ]);
    });
