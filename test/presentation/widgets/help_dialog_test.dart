...
import 'package:filetagger/presentation/help_topics.dart'
    show HelpTab, helpTopicsOf, systemTagDescription;
...
void main() {
  testWidgets('도움말은 탭을 모두 내걸고 사용법 탭을 먼저 보인다', (tester) async {
    ...
    expect(
      find.text(commandOf(AppCommandId.help).label(koL10n)),
      findsOneWidget,
    );
    for (final tab in HelpTab.values) {
      expect(
        find.text(tab.label(koL10n)),
        findsOneWidget,
        reason: tab.label(koL10n),
      );
    }
    // 첫 탭의 첫 개념 설명이 이미 보인다.
    expect(find.text(helpTopicsOf(koL10n).first.title), findsOneWidget);
  });
  ...
  testWidgets('기능과 단축키 탭은 명령 라벨과 단축키를 카탈로그에서 보인다', (tester) async {
    ...
    expect(find.text(rescan.label(koL10n)), findsOneWidget);
  ...
  testWidgets('사용 팁 탭은 팁 제목을 늘어놓는다', (tester) async {
    ...
    expect(find.text(usageTipsOf(koL10n).first.title), findsOneWidget);
  });
  ...
  testWidgets('시스템 태그 탭은 태그 이름과 설명을 보인다', (tester) async {
    ...
    expect(find.text(systemTagName(koL10n, first)), findsOneWidget);
    expect(find.text(systemTagDescription(koL10n, first)), findsOneWidget);
  });
  ...
  testWidgets('탭 막대를 눌러 다른 탭으로 옮겨 갈 수 있다', (tester) async {
    ...
    // 하나로 둔 이유다.
    await pumpHelp(tester);
    await tester.tap(find.text(HelpTab.tips.label(koL10n)));
    await tester.pumpAndSettle();
